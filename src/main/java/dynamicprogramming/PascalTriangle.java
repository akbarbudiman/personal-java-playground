...
public class PascalTriangle {
	...
	static int factorial(int number) {
		Object resultInMapping = factorialMapping.get(number);
		if (resultInMapping != null) return (int)resultInMapping;
	...
	static int[] createPascalTriangleLine(int lineNumber) {
		int[] result = new int[lineNumber];
		result[0] = 1;
	...
	public static void main(String[] args) {
		...
		int[] pascalTriangeLine = createPascalTriangleLine(lineNumber);
		
		for (int element : pascalTriangeLine) {
